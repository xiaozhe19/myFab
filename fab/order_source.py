...
class LotSource(Protocol):
    ...
    def initial_events(
        self, start_time: float, end_time: float, release_interval: float
    ) -> tuple[SourceEvent, ...]:
...
def _pop_or_remove(queue: deque[LotState], lot_id: str | None) -> LotState | None:
    """从 release pool 弹出队首或按 ID 移除一个候选 lot。"""
    if not queue:
        return None
    if lot_id is None:
        return queue.popleft()
    lot = next((candidate for candidate in queue if candidate.id == lot_id), None)
    if lot is None:
        return None
    queue.remove(lot)
    return lot
...
class SyntheticOrderSource:
    ...
    def initial_events(
        self, start_time: float, _: float, __: float
    ) -> tuple[SourceEvent, ...]:
        return (
    ...
    def handle_event(
        ...
        if kind is EventKind.ORDER_ARRIVAL:
            ...
            events = (
                (SourceEvent(next_time, EventKind.ORDER_ARRIVAL, {}),)
    ...
    def generate_order(self, time: float) -> None:
        if (
            self.spec.waiting_capacity
            and len(self.queue) + self.spec.lots_per_order > self.spec.waiting_capacity
        ):
            return
        ...
        for _ in range(self.spec.lots_per_order):
            ...
            self.queue.append(
                LotState(
                    id=f"L{self.generated_count:04d}",
                    product_id=product_id,
                    order_id=f"O{self.orders_generated:04d}",
                    generation_time=time,
                    release_time=float("inf"),
                    due_time=(
                        time + self.spec.due_time_offset
                        if self.spec.due_time_offset
                        else None
                    ),
                    priority=1,
                    route=self.model.products[product_id].route,
                    input_order=self.generated_count,
                    planned_release_time=time,
                    ready_time=float("inf"),
                )
            )

    def release(self, time: float, lot_id: str | None = None) -> LotState | None:
        lot = _pop_or_remove(self.queue, lot_id)
        if lot is not None:
...
class ExternalLotReleaseSource:
    ...
    def initial_events(
        ...
        arrivals = tuple(
            SourceEvent(
                max(start_time, spec.start_date),
                EventKind.LOT_AVAILABLE,
                {"plan_index": index},
            )
    ...
    def handle_event(
        ...
        if kind is EventKind.LOT_AVAILABLE:
            ...
            events = (
                (
                    SourceEvent(
                        next_time, EventKind.LOT_AVAILABLE, {"plan_index": index}
                    ),
                )
                if next_time < end_time
                else ()
    ...
    def _generate_release(self, index: int, spec: LotReleaseSpec, time: float) -> None:
        ...
        for _ in range(spec.lots_per_release):
            ...
            self.queue.append(
                LotState(
                    id=f"E{self._generated_count:06d}",
                    ...
                    wafer_count=lot_type.wafer_count,
    ...
    def _validate_plans(self) -> None:
        for spec in self.model.lot_releases:
            ...
            if product.route_name is not None and product.route_name != spec.route_name:
                raise ValueError(
                    f"Lotrelease 的路线 {spec.route_name} 与产品 {product.id} 不一致。"
                )
            if spec.lot_type_id not in self.model.lot_types:
                raise ValueError(
                    f"Lotrelease 引用了未知 lot type：{spec.lot_type_id}。"
                )
...
def create_lot_source(model: FabModel, random_seed: int) -> LotSource:
    ...
    raise ValueError(
        f"未知投料模式：{model.source_mode}；仅支持 synthetic 或 external。"
    )
